...
void graphics_init() {
    ...
    printf("fb1=%p fb2=%p frame_buffer=%p\n",
           fb1, fb2, frame_buffer);

    lcd_init();

    printf("AFTER lcd_init: fb1=%p fb2=%p frame_buffer=%p\n",
           fb1, fb2, frame_buffer);
}
...
void draw_rectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t colour) {
  ...
  // for (int j=y; j<y+h; j++) {
  //   printf("draw_rectangle: frame_buffer=%p fb1=%p fb2=%p j=%d\n",
  //          frame_buffer, fb1, fb2, j);
...
void cls(uint16_t colour) {
  printf("CLS: frame_buffer=%p fb1=%p fb2=%p\n",
         frame_buffer, fb1, fb2);
