...
volatile bool lcd_transfer_done = true;

static bool IRAM_ATTR lcd_color_trans_done(
...
void send_frame() {
  ...
  // Make absolutely sure the previous DMA transfer has finished before
  // touching/sending this framebuffer again.
  while (!lcd_transfer_done) {
    vTaskDelay(1);
  }
   ...
   * IMPORTANT:
   *
   * tx_color() is asynchronous. The DMA is still reading frame_buffer
   * after tx_color() returns.
   *
   * Since we only have one framebuffer, don't let the application modify
   * it until the transfer is actually finished.
   */
  while (!lcd_transfer_done) {
    vTaskDelay(1);
  }
